...
var H5ComponentPoint = function (name, cfg) {
    ...
    $.each(cfg.data, function (idx, item) {
        ...
        point.dom = $('<div class="point point_' + idx + '" >');
        var name = $('<div class="name" >' + item.name + '</div>');
        var rate = $('<div class="per" >' + item.per + '</div>');
        name.append(rate);
        point.dom.append(name);
        var per = ((item.per / base) * 100 ) + '%';

        point.dom.width(per).height(per);

        if (idx == 0) {
            point.dom.css('z-index', '10');
        }
        if (item.color) {
            point.dom.css('background-color', item.color);
        }

        point.dom.css({'transition':'all 1s '+idx*.5+'s',
